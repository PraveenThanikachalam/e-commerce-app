...
// .wrangler/tmp/bundle-kK3TFP/checked-fetch.js
...
var PgTable = class extends Table {
  /**@internal */
  [(_a115 = entityKind, InlineForeignKeys)] = [];
  /** @internal */
  [EnableRLS] = false;
  /** @internal */
  [Table.Symbol.ExtraConfigBuilder] = void 0;
};
__publicField(PgTable, _a115, "PgTable");
/** @internal */
...
_a122 = entityKind;
__publicField(PgDeleteBase, _a122, "PgDelete");
...
var wishlist = pgTable("wishlists", {
...
var WishlistRelations = relations(Wishlists, ({ many }) => ({
  WishlistItems: many(WishlistItems)
  // this creates a relationship between Wishlists and WishlistItems
}));
var WishlistItemsRelations = relations(WishlistItems, ({ one }) => ({
...
var isUserAlreadyExists = async (db, email) => {
  try {
    const userData = await db.select().from(void 0).where(eq((void 0).email, email));
...
var createUser = async (db, userInfo) => {
  ...
  try {
    const [user] = await db.insert(void 0).values({
      userId: validatedData.userId || "",
      name: validatedData.name || "",
      email: validatedData.email || "",
      role: validatedData.role || "USER",
      provider: validatedData.provider || "",
      avatarUrl: validatedData.avatar || ""
    }).returning({ name: (void 0).name, email: (void 0).email });
...
var UpdateUser = async (c3) => {
  ...
  if (c3.req.method === "PATCH") {
    try {
      ...
      if (User?.isExists) {
        const response = await db.update(void 0).set({
          name: validatedData.name,
          avatarUrl: validatedData.avatar,
          mobileNumber: validatedData.mobileNumber
        }).where(eq((void 0).id, User.userId));
...
// .wrangler/tmp/bundle-kK3TFP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-kK3TFP/middleware-loader.entry.ts
