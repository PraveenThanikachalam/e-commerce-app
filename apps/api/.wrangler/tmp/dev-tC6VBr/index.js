...
// .wrangler/tmp/bundle-hT4VRc/checked-fetch.js
...
__export(schema_exports, {
  ...
  users: () => users,
  wishlistItemRelations: () => wishlistItemRelations,
...
var wishlists = pgTable("wishlists", {
  ...
  userId: integer("userId").notNull().references(() => users.userId, { onDelete: "cascade" }).unique()
});
...
var wishlistItems = pgTable(
  "wishlistItems",
  {
    wishlistId: integer("wishlistId").notNull().references(() => wishlists.wishlistId, { onDelete: "cascade" }),
    productId: integer("productId").notNull().references(() => products.productId, { onDelete: "cascade" })
  },
  (t2) => ({
    pk: primaryKey({ columns: [t2.productId, t2.wishlistId] })
  })
);
var wishlistItemRelations = relations(wishlistItems, ({ one }) => ({
  wishlist: one(wishlists, {
...
var WishlistValidation = z.object({
...
HomepageRouter.get("get-data", verifyAuth(), GetHomePageContents);
...
var createUser = async (db, userInfo) => {
  ...
  try {
    const [user] = await db.insert(users).values(validatedData).returning({ name: users.name, email: users.email });
...
UserRouter.post("create-wishlist", createWishlist_default);
...
// .wrangler/tmp/bundle-hT4VRc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hT4VRc/middleware-loader.entry.ts
